...
const images = {
  ...
  out: '/images/alfa/out-details-marks.jpg',
};

const prompt =
  'Добавь объекты из второго изображения точно в зоны, отмеченные желтым. Карту не меняй, сохрани перспективу, материалы и общую композицию.';

export const ImageAnnotationsSlide: React.FC = () => {
  return (
      ...
      <div className="w-[400px] h-full flex flex-col justify-center pr-8 pt-16">
        <AnimatedBlock delay={0.2}>
          <p className="font-mono text-2xl text-retro-dim leading-[1.25]">
            Пометки прямо на изображении — модель читает зону вмешательства по разметке.
          </p>
          <div className="border-l-4 border-retro-accent pl-6 mt-6">
            <p className="font-mono text-xl text-retro-dim leading-[1.3]">{prompt}</p>
          </div>
        </AnimatedBlock>
      </div>
